...
const scriptsMgr = function () {
    return gulp.src([
        ...
        .pipe(concat('crosslinks.min.js'))
        .pipe(uglify())
        .pipe(header(banner + '\n', {pkg: pkg}))
        .pipe(gulp.dest('assets/components/crosslinks/js/mgr/'))
};
gulp.task('scripts', gulp.series(scriptsMgr));

const sassMgr = function () {
    return gulp.src([
        'source/sass/mgr/crosslinks.scss'
    ])
        ...
        .pipe(footer('\n' + banner, {pkg: pkg}))
        .pipe(gulp.dest('assets/components/crosslinks/css/mgr/'))
};
gulp.task('sass', gulp.series(sassMgr));

const imagesMgr = function () {
    return gulp.src('./source/img/**/*.+(png|jpg|gif|svg)', {encoding: false})
        .pipe(gulp.dest('assets/components/crosslinks/img/'));
};
gulp.task('images', gulp.series(imagesMgr));

const bumpCopyright = function () {
    return gulp.src([
        'core/components/crosslinks/model/crosslinks/crosslinks.class.php',
        'core/components/crosslinks/src/Crosslinks.php',
    ], {base: './'})
        .pipe(replace(/Copyright 2018(-\d{4})? by/g, 'Copyright ' + (year > 2018 ? '2018-' : '') + year + ' by'))
        .pipe(gulp.dest('.'));
};
...
const bumpHomepanel = function () {
    return gulp.src([
        ...
        .pipe(replace(/&copy; 2018(-\d{4})?/g, '&copy; ' + (year > 2018 ? '2018-' : '') + year))
        .pipe(gulp.dest('.'));
};
